...
class Plotter:
    ...
    def plot_day(self, *, year, month, day, fig, ax,savePlot=False,fileName='daySessionOut.png',drawColor='cyan',backgroundColor=(0.1, 0.2, 0.5)):
        fig.patch.set_facecolor(backgroundColor)
        if (year, month, day) not in self.days:
            raise TypeError(f"No data from {year}-{month}-{day}!")
        day = self.days[(year, month, day)]
        for route in day.routes():
            ax.plot(*route, "k", linewidth=1, antialiased=True,color=drawColor)
    ...
    def plot_month(self, *, year, month, fig, ax, extra_offset=np.array([0, 0]),savePlot=False,fileName='daySessionOut.png',drawColor='k',backgroundColor='w'):
        fig.patch.set_facecolor(backgroundColor)
    ...
    def plot_year(self, *, year, fig, ax, n_cols=4, month_gap=0, col_gap=0.5,savePlot=False,fileName='daySessionOut.png',drawColor='k',backgroundColor='w'):
        fig.patch.set_facecolor(backgroundColor)
        n_rows = 12 // n_cols
        y_offset = 0
        new_y_offset = 0
